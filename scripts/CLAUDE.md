...
`verify-mobile-portrait-v12.mjs`: 390×844 v1.4 真实触控门禁（20 断言，保留旧文件名）：开球触摸落位、双方水平、辅助线首次关闭/灯泡开启/刷新持久化、10px 手抖下的长按接管与同边换位、五控件合成层逐帧拖动、拖动期存储节流、布局无数值偷跑、拨轮保留沿边落点的右/底吸附与横竖原操作、三布局存储及无错误。
