...
`verify-precision-aim.mjs`: 瞄准输入出口门禁（14 断言）：开球虚母球、落位后默认左右键、灯泡三入口、预测线独立开关、拨轮显式开启、无辅助信息泄露、固定普通档/精调离合与页面无错误。
...
`verify-mobile-portrait-v12.mjs`: 390×844 真实触控门禁（53 断言）：开球虚母球、默认左右键、灯泡三入口不溢出、按需拨轮/64px 精调离合，并覆盖连续视角、54px 黑轨编辑、击球点、满力、走位与复盘；截图目录可由 `SHOT_DIR` 覆盖。

`verify-position-plan.mjs`: 💡 菜单内的走位规划 + 击球复盘集成回归（34 断言）：默认熄灭，主动点亮走位入口后只显示最高概率方案与当前一杆，点击第 2/3 杆才切换路线；真实出杆结算后复盘保持隐藏，再次开启走位复盘入口才出现复盘 chip 与场景对比。
