...
`verify-pockets.mjs`: 共享袋口几何视觉门禁：先断言一体式木框内沿切出六袋、四个角袋圆润收肩、外轮廓只有四个圆角、袋口无软厚外凸包边，且六袋均具备浅驼色薄皮外护口/深色袋唇/椭圆暗口/斜壁/暗底；角袋袋唇还须精确跨过八个角衬锚点并向库边内隐藏至少 7mm；再固定桌面俯视、角袋近景、中袋近景与 390×844 竖屏四机位，输出 `shots/pocket-*.png` 并拒绝页面错误或横向溢出。
