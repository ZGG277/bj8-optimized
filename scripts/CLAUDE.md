...
`verify-precision-aim.mjs`: 无限拨轮出口门禁（8 断言）：开球初始实体/虚母球均隐藏、合法区域虚球跟手、点击落实并呼出拨轮、非袋口与近袋均保持粗档、轻点显式进入精瞄、固定低速横拨改世界杆向与页面无错误；俯视端坐标按当前连续环绕相机目标位姿投影。
...
`verify-pockets.mjs`: 共享袋口几何视觉门禁：先断言一体式木框内沿切出六袋、四个角袋圆润收肩、外轮廓只有四个圆角、袋口无粗黑软圈/外凸包边，且六袋均具备 7–8mm 台内圆弧凹口、≥15mm 宽/约 6mm 高的浅驼皮圈、≤1.8mm 细缝边、六层十八股白色菱形网、斜壁与暗底；角袋缝边还须精确跨过八个角衬锚点并向库边内隐藏至少 7mm；再固定桌面俯视、角袋近景、中袋近景与 390×844 竖屏四机位，输出 `shots/pocket-*.png` 并拒绝页面错误或横向溢出。
