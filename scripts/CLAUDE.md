...
`verify-precision-aim.mjs`: 默认拨轮/可选方向键出口门禁：开球白球在开球线中点标准位自动实体就位、按住拖到合法开球区、越线保留最后合法位，灯泡四入口、方向键单击/长按、切回拨轮、非袋口与近袋保持粗档、轻点显式精瞄、固定低速横拨改世界杆向与页面无错误。
...
`verify-pockets.mjs`: 共享库边/袋口几何视觉门禁：断言一体式木框六袋、共享凹口、六条 ≥19mm 宽/约 7mm 高且下沉木帮的浅驼护口、12 个顶盖 jaw 锚点、六条同曲线对齐皮裙、细缝边、白色菱形网、斜壁与暗底；再固定桌面俯视、角袋近景、中袋近景与 390×844 竖屏四机位，输出 `shots/pocket-*.png` 并拒绝页面错误或横向溢出。
