...
走位规划不是玄学：单杆物理已完全解析化（90°/30° 规则），清台顺序有约 30 条成文法则，学术界（PickPocket/CueCard，ICGA 台球奥赛）二十年前就用「确定性物理仿真 + 蒙特卡洛采样 + 2 层前瞻搜索」打赢人类。本项目已有确定性 240Hz 物理内核；2026-08-13 重新测量当前完整碰撞模型后，单杆基线约 4–5ms，而非早期文档记录的 0.15ms，因此产品搜索必须继续限预算并由 Worker 承担。多杆走位规划是商业产品空白，拍照还原球局已被 pix2pockets（arXiv 2025）打通技术但无消费级产品——两个方向都是真空白、真差异化。
...
- **事件驱动仿真**（Greenspan，Deep Green，IEEE Computer 2008）：解析预测下次碰撞时刻，比定步长快 2–3 个数量级。当前约 4–5ms/杆仍先采用活跃球配对、几何粗筛、Worker、早停和预算截断；只有这些手段不足时才评估迁移，避免先重写确定性物理。
...
- **实时性账（2026-08-13 复测）**：活跃球配对后当前 240Hz 全杆仿真约 4.1ms、120Hz 约 2.0ms、60Hz 约 1.1ms。产品入口采用 320 次、两层、2.5 秒截止；`search.ts` 的 8000 次默认值只属于显式深度调用。手段按性价比：几何粗筛 → Web Worker → 早停/预算截断 → 粗算后 240Hz 复核，暂不直接迁移 WASM/事件驱动。
...
球局变化（进入玩家回合）→ usePositionPlan 检测世界指纹变化
  → planPositionAsync（Web Worker，generation 令牌防陈旧，失败降级主线程）
    ...
    → search.ts      精确终态滚动前瞻：第 2/3 步各从上一步无扰动展示仿真的
                     ...
                     标定失败即递补，全灭则链条截断；产品 simBudget=320 截断
...
- benchmark（`src/planner/bench.test.ts`，2026-08-13）：240Hz 全杆仿真约 4.1ms；120Hz 约 2.0ms；60Hz 约 1.1ms；测试以 20ms/杆和 0.2ms/clone 的宽松阈值阻止数量级回退。
- 产品入口固定 320 次、两层、2.5 秒墙钟截止；8000 次仅为规划库显式深度调用的默认上限，不再代表 UI 产品预算。
...
- **点亮才预算**：只有玩家主动点亮灯泡且球局静止时才启动 Worker；默认状态不做后台规划。按钮三态见 §2.5（computing 呼吸 → ready 微光，无可行方案灰显不隐藏）。
...
- **独立语义**：瞄准线、走位规划、击球复盘和瞄准器各自切换。复盘默认关闭；关闭复盘会同时收起已展开的轨迹对比，且不会启动规划 Worker。

### 2.6 顾燃训练闭环（2026-08-13 升级）

- **捕获点**：`handleCommit` 在 `strikeCueBall` 前快照世界、angle/power/spin，并用当前杆向推断目标球/袋口；只有玩家主动看过规划时才附带 `plans[0].steps[0]`。对手杆走 useOpponentAI，不进入玩家训练样本。
- **自主复盘**：未开规划也会在结算后精确回放一次。纠正优先级固定为犯规 → 未进球厚薄/偏侧 → 洗袋 → 袋口边缘/力度 → 稳定动作，一杆只显示一个建议。开球没有目标袋时根据犯规与落袋事实给开球建议。
- **计划对比**：主动查看过规划的杆保留原有 zone、力度、杆法和计划/实际轨迹判定；未查看规划的杆 `planned=null`，只显示自己的赛后诊断，不画系统答案轨迹。陪练与挑战都可由玩家主动开启规划；计算中和无可靠直攻路线时必须给出显式状态反馈。
- **整局总结**：玩家画像仍在胜负确定后一次提交；结算页只选犯规控制、准度、母球走位、稳定发挥中的一个重点，并给一个下一局目标。
- **UI**：复盘数据每杆生成但默认收起；从灯泡打开复盘后，`.review-chip` 统一以“复盘”开头，不使用顾燃口吻。有计划的复盘才显示「▶ 对比」并允许展开俯视轨迹。规划讲下一杆、复盘讲上一杆，planOpen 时复盘 chip 让位。
