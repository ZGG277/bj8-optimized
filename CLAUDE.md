...
独立 Vite + React + Three.js 中式八球实验场（产品名「瓜瓜台球」），验证 240 Hz 物理、shot/tactical/spectator 显式相机、杆法、规则轮转与真实交互；指针瞄准在单次手势内冻结活相机，幽灵球近母球时以两球直径、10% 迟滞带与快速跨心检测消除角度奇点；母球放置、点台落位或幽灵球落位自动进入对齐杆向的第一人称，抓瞄准线粗调不转场；出杆保持触球构图 300ms 后自动抬升全台观察结果。移动端采用 1.5×/1024 阴影节能预算，React→Three.js 拆分 world/aim/visibility/camera 去耦通道，纯镜头操作不重放世界、球杆或阴影。台呢与袋口材质以固定种子生成纵向梳毛、中性皮纹与袋腔暗口，视觉回归可复现且不改物理袋口。走位只在用户显式开启“走位与击球复盘”开关后计算；共享袋口几何仍采用角袋 100mm/中袋 102mm 与 8/7mm 台内捕获弧。
...
`scripts/`: 构建发布适配、浏览器交互、画面与场景对象验证脚本；含妙搭单 HTML 门禁、真实指针回归（48 断言）、手机竖屏触控防抖/五控件换位/压感拨轮与流畅拖放（23 断言）、shot/tactical/spectator 生命周期、环绕、锁镜与通道耗能（15 断言）、调色盘/控件显隐与三主题三视口矩阵（26 断言）、虚母球/默认精瞄无限拨轮（8 断言）、预测球路一致性（19 断言）及规划/规则/视觉专项，成员清单见 `scripts/CLAUDE.md`。
