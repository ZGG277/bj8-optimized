...
独立 Vite + React + Three.js 中式八球实验场（产品名「瓜瓜台球」），验证 240 Hz 物理、连续环绕视角、杆法、规则轮转与真实交互；移动端采用 1.25×/1024 阴影节能预算，静止球桌按需渲染，走位只在用户点亮灯泡后计算；顾燃的杆向容错按母球实际视角计算并压住高斯长尾。共享袋口几何采用角袋 100mm/中袋 102mm，并以 8/7mm 台内圆弧统一视觉凹口与连续落袋捕获，HUD 提供首局微提示、动态实力、手动粗/精瞄、可停靠五控件及三套主题表达。
...
`scripts/`: 构建发布适配、浏览器交互、画面与场景对象验证脚本；含妙搭单 HTML 门禁、真实指针回归（41 断言）、手机竖屏触控防抖/五控件换位/压感拨轮与流畅拖放（23 断言）、对手观战/玩家手动与全局相机/触屏锁镜及控件避让（14 断言）、调色盘/控件显隐与三主题三视口矩阵（26 断言）、虚母球/无限拨轮（8 断言）、预测球路一致性（19 断言）及规划/规则/视觉专项，成员清单见 `scripts/CLAUDE.md`。
