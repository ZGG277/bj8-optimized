...
`scripts/`: 构建发布适配、浏览器交互、画面与场景对象验证脚本；含妙搭单 HTML 门禁、真实指针回归（41 断言）、手机竖屏触控防抖/五控件换位与流畅拖放（20 断言）、对手观战/玩家全局/触屏锁镜与控件避让（11 断言）、调色盘与三主题三视口矩阵（24 断言）、虚母球/无限拨轮（8 断言）、预测球路一致性（19 断言）及规划/规则/视觉专项，成员清单见 `scripts/CLAUDE.md`。
