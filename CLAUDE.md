...
独立 Vite + React + Three.js 中式八球实验场（产品名「瓜瓜台球」），验证 240 Hz 物理、连续环绕视角、杆法、规则轮转与真实交互；共享袋口几何采用角袋 100mm/中袋 102mm，并以 8/7mm 台内圆弧统一视觉凹口与连续落袋捕获，HUD 提供只由真实动作推进且可跳过的首局一行微提示、三杆动态实力、默认关闭的预测辅助线、显式轻点切换的固定精瞄拨轮、视角控件内手动相机入口、静置半透明/按住实色的无框可停靠五控件及三套主题表达。
