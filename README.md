...
零出杆记录的新用户首次开始对局会运行一次可跳过的首局引导：按放置白球、开球粗瞄/精瞄、下拉蓄力、顾燃观战、玩家观察球形、视角、母球击球点、出杆与长按移动控件分段提示。提示气泡跟随对应控件并主动避让触控区，滚动阶段自动隐藏；完成、跳过或首局结束后写入本机，不再重复打扰，并保留键盘操作与 aria 语义。
...
- 首局引导：`src/first-match-guide.test.ts`（步骤推进、越级防护、完成/跳过持久化与受限存储回退）
