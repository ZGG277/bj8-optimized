...
Vite + React + Three.js 的 3D 中式八球（黑八）单机游戏：240 Hz 确定性物理内核、纯规则状态机、第一人称/俯视共享世界杆向、360° 粗瞄 + 近袋局部精瞄、杆法（高低杆与加塞）、动态实力 AI「顾燃」、💡 走位规划（蒙特卡洛 3 层前瞻搜索连续 1–3 杆方案，含清组转 8 号、轨迹播放与走位区域展示）。

v1.2.0 采用手机竖屏优先 HUD：34px 单行球组状态、右侧单手控制轨、球桌内方向微调、力度/出杆合一长控件、可展开击球点与可拖动规划/复盘浮层。

当前对局提供两种关系：**陪练**按玩家长期能力匹配略弱对手，保留走位/复盘；**挑战**匹配高一档对手并关闭赛中规划提示。玩家能力只从目标明确的非开球样本更新，按球路容错归一化难度并带置信度；AI 档案在每局开始生成后锁定，局内不会追着比分升降。
...
| `scripts/verify-precision-aim.mjs` | 360° 粗瞄 + 近袋精瞄双档交互（5 断言） |
...
```bash
npm run build
lark-cli apps +html-publish --app-id app_17atze3ahyj --path ./dist --as user
lark-cli apps +release-get --app-id app_17atze3ahyj --release-id <release_id>   # 轮询至 finished
```
