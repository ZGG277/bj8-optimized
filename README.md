...
> **历史归档快照（2026-08-31）。** 此工作树只用于保留早期 `ghost-cue-aim-dial` 实验与验收证据，不再开发、构建或发布。当前权威源码为 GitHub `ZGG277/bj8-optimized` 的 `main` / `v1.4.0`。以下内容是归档时的历史说明。

Vite + React + Three.js 的 3D 中式八球（黑八）单机游戏：240 Hz 确定性物理内核、纯规则状态机、可停留任意高度的第一人称→俯视连续环绕视角、360° 球桌粗瞄 + 默认球杆左右微调、灯泡按需精瞄拨轮、杆法（高低杆与加塞）、动态实力 AI「顾燃」与走位/复盘。

v1.2.0 采用零浪费 HUD：桌面删除产品/回合/杆数顶栏，手机端只保留 34px 单行球组状态；右侧四个单手控件等宽，视角采用可停任意位置的长行程推杆。瞄准默认使用球杆左右键；灯泡展开“瞄准辅助线 / 走位复盘 / 拨轮瞄准”三个小入口，第三项打开后才以固定双档无限拨轮取代左右键。
...
- 瞄准偏好/拨轮：`src/hooks/useAimAssist.test.ts` + `src/input/aim-dial.test.ts`（默认关闭、容错持久化、固定普通/精调双档与无边界方向）
...
| `scripts/verify-precision-aim.mjs` | 开球虚母球跟手/落实、默认左右键、灯泡三入口、预测线独立开关、按需拨轮与固定双档（14 断言） |
...
| `scripts/verify-mobile-portrait-v12.mjs` | 竖屏虚母球、默认左右键、灯泡三入口/按需拨轮、连续视角、整轨编辑、击球点、走位与复盘（53 断言） |
