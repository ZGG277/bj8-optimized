...
手机端默认采用 1.25× 像素比上限、1024 阴影贴图与低功耗 GPU 偏好；球桌静止且相机/动画收敛后停止空转 rAF，只有世界、镜头或控件实际变化才唤醒下一帧。走位 Worker 仅在用户点亮灯泡后启动，避免默认关闭提示时仍进行 8000 次后台仿真。顾燃的瞄准容错统一换算到母球真实杆向，并把随机执行误差连续限制在袋口安全窗口内，保留实力差异但不再出现明显瞄出口外的长尾坏杆。
