...
`aim-dial.ts`: 无限横/竖拨轮的纯传动换算；远袋约 900px/周，进入 3.8× 有效袋口窗口后 smoothstep 连续降档，精瞄时约 180px 横跨完整袋口；可靠 Pointer 压力越高则传动越紧，固定 0.5 普通触摸和鼠标回退原速度。

`aim-dial.test.ts`: 粗档、接近区连续降速、精瞄全袋口行程、压力到增益/视觉状态与无压感回退回归。
