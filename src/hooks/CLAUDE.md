...
`useAimInteraction.ts`: 交互协调层，封装跟手虚母球、Pointer 落位、点哪打哪、抓影子球与 360° 粗瞄；屏幕反算显式消费当前视觉相机方位，因此顾燃交棒后的自由高位镜头不会与幽灵球点击错位；母球或幽灵球落位后立即呼出无限拨轮，并只在拖动结束且世界角真实变化时上报粗瞄事实，拨轮调整返回实际角度是否变化。

`useOpponentAI.ts`: AI 调度层，副作用 Hook——match.phase === 'opponent' 时消费最新有效档案；500ms 自然停顿后最多等待后台规划 2s，超时取消并回退轻量选杆，使出杆思考约束在 3s 内；同时调度选杆扰动、执行误差、击球与出杆动画。
