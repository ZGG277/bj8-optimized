...
`useGameState.ts`: 状态管理收敛层，集中管理 Game 组件的所有状态（worldView/match/viewLevel）与 ref（worldRef/matchRef），提供 resetGame/settleShotRaw/setMessage 等编排动作；新局同步重置 shot 结算守卫。
