...
[OUTPUT]: 断言一行提示只保留弱化跳过入口、默认精瞄拨轮文案且不回归手动下一步/完成控件
...
describe('FirstMatchGuide 微提示结构', () => {
  ...
  it('精瞄提示直接引导轻拨，不要求先切档', () => {
    ...
    expect(markup).toContain('轻拨拨轮·精细瞄准');
    expect(markup).not.toContain('先切档');
    expect(markup).not.toContain('轻点拨轮');
  });
});
