...
`components/`: HUD 控制组件（桌面/竖屏长行程视角推杆、球桌内无限横向拨轮、可展开击球点、力度出杆合一控件、单行球组状态、可拖动规划/复盘浮层、球桌视口与开始界面），只转发事件，不持有对局状态；产品/回合/杆数顶栏已移除。
...
`Scene3D.ts`: Three.js 场景适配器，双视角共享唯一世界杆向，第一人称杆/视线/球路共线；瞄准辅助共享 physics 恢复/throw 预测并按障碍/库边裁剪；摆球阶段隐藏实体母球、只显示跟手半透明预览，落实后恢复实体；另负责台面↔屏幕映射、合法目标环、球杆、走位/复盘与 rAF 相机平滑。
