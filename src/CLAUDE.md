...
`Game.tsx`: 对局编排器，规则在 `match/`、输入在 `input/`、控件在 `components/`、物理时钟在 `simulation/`；剩余职责为 AI 调度、视角/HUD 状态编排（含俯身 camLift 微调）与瞄准映射（点空点=靶点落此、抓影子球=拖到台面任意位、抓瞄准线=360° 转角保距离，角度全周无钳制，出杆后靶点回自动）。
...
`Scene3D.ts`: Three.js 场景适配器，消费物理世界快照并提供双视角（第一人称杆后共线）、合法目标环、球杆动画（杆身 yaw 取负与瞄准反向对齐，防镜像拨球）、瞄准辅助（射线/分离线/可点击定位的幽灵球靶点）、自由球预览和屏幕到球桌坐标映射；袋口按真实中八构造（圆鼻角衬、皮口唇边喉管、袋腔背壁、角袋金属件），视觉开口与物理判定解耦；相机平滑收敛随 rAF 每帧推进，不依赖 React 渲染节奏。

`physics.ts`: 以米为单位的 240 Hz 确定性二维台球内核，输出首碰、碰库与落袋事件，禁止依赖 React、DOM 或规则状态；摆球无参固定（测试复现）、传 rng 随机（8 号居中、底角一全一花、其余乱序，消除分组偏向）。
