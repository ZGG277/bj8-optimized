...
const url = 'https://lg22l37ytz.feishuapp.com/app/app_17b18dh5axj';
const root = new URL('../../', import.meta.url);
const out = new URL('./', import.meta.url);
const expectedCommit = execFileSync('git', ['rev-parse', 'v1.7.0^{}'], {
...
const report = { url, startedAt: new Date().toISOString(), expectedCommit,
  sourceTag: 'v1.7.0', expectedHash, loginState: 'fresh browser contexts, no supplied cookies or login', results };
...
try {
  ...
  for (const viewport of [
    ...
    try {
      ...
      check(`${viewport.name} 线上来源提交与标签一致`, provenance.sourceCommit === expectedCommit
        && provenance.sourceTag === 'v1.7.0' && provenance.artifactSha256 === expectedHash, provenance);
      await frame.waitForSelector('.start-btn');
      const beginner = await frame.evaluate(() => ({
        player: document.querySelector('.skill-summary strong')?.textContent ?? '',
        opponents: [...document.querySelectorAll('.mode-choice small')].map(node => node.textContent ?? ''),
        worlds: document.querySelectorAll('.world-choice input[name="world"]').length,
      }));
      check(`${viewport.name} 首次双方 25 且五世界正式可选`, beginner.player.includes('25')
        && beginner.opponents.length === 2 && beginner.opponents.every(text => text.includes('25'))
        && beginner.worlds === 5, beginner);
  ...
  report.passed = results.length === 20 && results.every(result => result.pass);
